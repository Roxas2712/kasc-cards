...
DESTINATIONS=[('kasc-cards','v1.3.2'),('kanto-ascendant','v6.7.17'),('voxel-ascendant','v3.0.34')]
def get(url):
...
if __name__=='__main__':
    ...
    for repo,tag in DESTINATIONS:
        ...
        for name in ['kasc-card-index.json','SHA256SUMS.txt']:
            remote_name = 'KASC-Cards-1.3.2-SHA256SUMS.txt' if name == 'SHA256SUMS.txt' and repo != 'kasc-cards' else name
    ...
    assert latest['tag_name']=='v1.3.2' and not latest['prerelease'] and not latest['draft']
