...
for _,ed in ipairs({'red','blue','yellow'})do
 local before=assert(C.decode(read(repo..'/cards/kanto-ascendant-'..ed..'-1.3.1.g1rcart')))
 local after=assert(C.decode(read(repo..'/cards/kanto-ascendant-'..ed..'-1.3.2.g1rcart')))
 assert(after.version=='1.3.2' and C.publishable(after));assert(C.labelArtBytes(before)==C.labelArtBytes(after))
 ...
 assert(row and row.version==after.version and row.downloadURL:find('v1.3.2/',1,true))
 for i,pin in ipairs(after.mods)do
  local expected=pin.id=='kanto_ascendant' and '6.7.17' or '3.0.34'
  ...
  assert(release.zip.url:find(pin.id=='kanto_ascendant' and '/v6.7.17/' or '/v3.0.34/',1,true))
 end
...
assert(not Sem.satisfies('3.0.34',spec),'new VASC blocked')
...
for _,v in ipairs({'3.0.0-rc.15.1','3.0.0-rc.16','3.0.0','3.0.5','3.0.34','4.0.0','9.0.0-rc.1','9.0.0'}) do
...
for _,v in ipairs({'3.0.0-rc.15.1','3.0.5','3.0.34','4.0.0','9.0.0-rc.1','9.0.0'}) do
