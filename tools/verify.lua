...
for _,ed in ipairs({'red','blue','yellow'})do
 local before=assert(C.decode(read(repo..'/cards/kanto-ascendant-'..ed..'-1.3.0-rc.4.g1rcart')))
 local after=assert(C.decode(read(repo..'/cards/kanto-ascendant-'..ed..'-1.3.0-rc.5.g1rcart')))
 assert(after.version=='1.3.0-rc.5' and C.publishable(after));assert(C.labelArtBytes(before)==C.labelArtBytes(after))
 ...
 assert(row and row.version==after.version and row.downloadURL:find('v1.3.0-rc.5/',1,true))
 for i,pin in ipairs(after.mods)do
  local expected=pin.id=='kanto_ascendant' and '6.7.0-rc.5' or '3.0.0-rc.15.1'
  ...
  assert(release.zip.url:find(pin.id=='kanto_ascendant' and '/v6.7.0-rc.5/' or '/v3.0.0-rc.15.1/',1,true))
 end
...
assert(not Sem.satisfies('3.0.0-rc.15.1',spec),'new VASC blocked')
for _,v in ipairs({'3.0.0-rc.12','3.0.0-rc.13','3.0.0-rc.14','3.0.0-rc.15'})do assert(not Sem.satisfies(v,spec))end
for _,v in ipairs({'3.0.0-rc.15.0','3.0.0-rc.15.2','3.0.0-rc.16','3.0.0'})do assert(Sem.satisfies(v,spec),'untested VASC admitted: '..v)end
assert(Sem.compare('3.0.0-rc.15.1','3.0.0-rc.15')>0,'update indistinguishable')
local f=assert(io.open(repo..'/verification.json','w'));f:write(assert(J.encode(report))..'\n');f:close()
print('PASS explicit KASC admission and distinct VASC update version; future versions remain blocked')
