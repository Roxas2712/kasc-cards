...
for _,ed in ipairs({'red','blue','yellow'})do
 local before=assert(C.decode(read(repo..'/cards/kanto-ascendant-'..ed..'-1.3.0-rc.14.g1rcart')))
 local after=assert(C.decode(read(repo..'/cards/kanto-ascendant-'..ed..'-1.3.0-rc.15.g1rcart')))
 assert(after.version=='1.3.0-rc.15' and C.publishable(after));assert(C.labelArtBytes(before)==C.labelArtBytes(after))
 ...
 assert(row and row.version==after.version and row.downloadURL:find('v1.3.0-rc.15/',1,true))
 for i,pin in ipairs(after.mods)do
  local expected=pin.id=='kanto_ascendant' and '6.7.0-rc.11' or '3.0.9'
  ...
  assert(release.zip.url:find(pin.id=='kanto_ascendant' and '/v6.7.0-rc.11/' or '/v3.0.9/',1,true))
 end
